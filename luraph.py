"""
Luraph v14.7 string dumper helper.
Source: https://github.com/2zvh/-
"""
...
def build(src_bytes: bytes, key: str = "") -> str:
    """
    Wrap raw Luraph source bytes into a Roblox executor-ready .luau string.
    Returns the full .luau content as a string.
    """
    key_str = key.strip() if key.strip() else "__PUT_YOUR_KEY_HERE__"
    # Escape backslashes and double-quotes so the key is valid inside a Lua string literal
    key_escaped = key_str.replace("\\", "\\\\").replace('"', '\\"')
    lua_template = LUAU_TEMPLATE.replace("%SCRIPT_KEY%", f'"{key_escaped}"')
    full = 'local _LPH_SRC = "' + src_bytes.hex() + '"\n' + lua_template
    return full
